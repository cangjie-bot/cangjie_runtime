...
struct CJThread *CJThreadAndArgsMemAlloc()
{
    ...
    if (ptr == nullptr) {
        HILOG_ERROR(ERRNO_SCHD_CJTHREAD_ALLOC_FAILED, "cjthread malloc failed");
        return nullptr;
    }
...
static char* StackMemAllocInternal(size_t allocSize)
{
    if (allocSize >= HUGE_PAGE) {
        ...
        if (stackAddr == MAP_FAILED) {
            HILOG_ERROR(ERRNO_SCHD_CJTHREAD_ALLOC_FAILED, "allocate cjthread stack with huge page failed");
            return nullptr;
        }
        ...
        if (stackAddr == nullptr) {
            HILOG_ERROR(ERRNO_SCHD_CJTHREAD_ALLOC_FAILED, "allocate cjthread stack failed");
            return nullptr;
        }
#ifdef CANGJIE_HWASAN_SUPPORT
        stackAddr =
    ...
    if (schedule->schdCJThread.stackProtect == false) {
       ...
       *totalSize = stackSizeAlign;
        ...
        *totalSize = pageSize + stackSizeAlign;
        ...
        if (addr == MAP_FAILED) {
            HILOG_ERROR(errno, "cjthread stack mmap failed, size is %u", *totalSize);
            return nullptr;
        }
#if defined (__linux__) || defined(__OHOS__) || defined(__ANDROID__)
        ...
        if (error) {
            HILOG_ERROR(errno, "cjthread stack mprotect failed, page_size: %u", pageSize);
                               ...
                               struct StackAttr *stackAttr, CJThreadBuf coBuf)
{
    ...
    if (newCJThread == nullptr) {
        HILOG_ERROR(errno, "cjthread malloc failed");
        return nullptr;
    }
    ...
    if (error) {
        HILOG_ERROR(error, "CJThreadInit failed");
                               ...
                               const void *argStart, unsigned int argSize, bool isSignal)
{
    ...
    if (newCJThread == nullptr) {
        HILOG_ERROR(ERRNO_SCHD_CJTHREAD_ALLOC_FAILED, "cjthread alloc failed");
        if (targetSchedule->scheduleType != SCHEDULE_DEFAULT) {
            atomic_fetch_sub(&scheduleCJThread->cjthreadNum, 1ULL);
        }
        return nullptr;
                           ...
                           const void *argStart, unsigned int argSize, bool isSignal)
{
    ...
    if (error) {
        ...
        HILOG_ERROR(error, "cjthread add to queue failed");
        return nullptr;
    }
...
{
    if (schedule == nullptr) {
        HILOG_ERROR(ERRNO_SCHD_INVALID, "schedule null invalid");
        return nullptr;
    }
...
int CJThreadPark(ParkCallbackFunc func, TraceEvent waitReason, void *arg)
{
    ...
    if (cjthread->schedule->scheduleType == SCHEDULE_UI_THREAD && cjthread->singleModelC2NCount > 0) {
        HILOG_WARN(ERRNO_SCHD_UITHREAD_ERROR, "parking cjthread with foreign function context on UI thread is not permitted.");
