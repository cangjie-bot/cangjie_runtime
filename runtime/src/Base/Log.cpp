...
void SignpostWrapper::IntervalBegin(const char* name) {
    ...
    if (name == nullptr || name[0] == '\0') {
        PRINT_WARN("IntervalBegin: name is nullptr or empty \n");
        name = "unnamedOperation";
    }
    GetName() = name;

    std::pair<size_t, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN, name);
...
void SignpostWrapper::IntervalEnd() {
    ...
    if (name == nullptr || name[0] == '\0') {
        PRINT_WARN("IntervalEnd: name is nullptr or empty \n");
        name = "unnamedOperation";
    }

    std::pair<size_t, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END, name);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    __builtin_os_log_format(buffer, INTERVAL_FORMAT_STR, name);
...
void SignpostWrapper::IntervalBeginAsync(const char* name, int32_t taskId) {
    ...
    if (name == nullptr || name[0] == '\0') {
        PRINT_WARN("IntervalBeginAsync: name is nullptr or empty \n");
        name = "unnamedOperationAsync";
    }

    std::pair<size_t, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN_ASYNC,
                                                      name, taskId);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
...
void SignpostWrapper::IntervalEndAsync(const char* name, int32_t taskId) {
    ...
    if (name == nullptr || name[0] == '\0') {
        PRINT_WARN("IntervalEndAsync: name is nullptr or empty \n");
        name = "unnamedOperationAsync";
    }

    std::pair<size_t, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC,
                                                      name, taskId);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    __builtin_os_log_format(buffer, INTERVAL_ASYNC_FORMAT_STR, name, taskId);
...
void SignpostWrapper::EventEmit(const char* name, int64_t count) {
    ...
    if (name == nullptr || name[0] == '\0') {
        PRINT_WARN("EventEmit: name is nullptr or empty \n");
        name = "unnamedOperationEmit";
    }

    std::pair<size_t, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_EVENT, name, count);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
