    ...
    switch (type) {
        case SignpostType::SIGNPOST_TYPE_EVENT: {
            if (value >= 0) {
                ...
                bufferSize = __builtin_os_log_format_buffer_size(NEG_NUM_FORMAT_STR, name, SIGNPOST_INT64(value));
            }
            break;
        }
        ...
        case SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC: {
            if (value >= 0) {
                ...
                bufferSize = __builtin_os_log_format_buffer_size(NEG_NUM_FORMAT_STR, name,
                                                                 SIGNPOST_INT32(static_cast<int32_t>(value)));
            }
            break;
        }
        default:
    ...
    switch (type) {
        case SignpostType::SIGNPOST_TYPE_EVENT: {
            if (value >= 0) {
                ...
                __builtin_os_log_format(buffer, NEG_NUM_FORMAT_STR, name, SIGNPOST_INT64(value));
            }
            break;
        }
        case SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN:
        ...
        case SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC: {
            if (value >= 0) {
                ...
                __builtin_os_log_format(buffer, NEG_NUM_FORMAT_STR, name, SIGNPOST_INT32(static_cast<int32_t>(value)));
            }
...
void SignpostWrapper::IntervalBeginAsync(const char* name, int32_t taskId) {
    ...
    emitWithNameImplFunc(&__dso_handle, osLog, OS_SIGNPOST_INTERVAL_BEGIN, spId, "AsyncOperation",
                         ((taskId >= 0) ? "name:%{public}@ taskId:%d" : PRINT_NEG_NUM_FMT),
