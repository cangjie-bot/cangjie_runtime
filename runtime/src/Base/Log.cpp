...
std::optional<std::pair<size_t, void *>> SignpostWrapper::FormatArgs(SignpostType type, const char* name, int64_t value) {
    size_t alignment = 16;
    size_t bufferSize = 0;
    void *buffer = nullptr;
    switch (type) {
...
void SignpostWrapper::IntervalBegin(const char* name) {
    ...
    auto retVal = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN, name);
    if (!retVal) {
        return;
    }
    auto bufferPair = retVal.value();
...
void SignpostWrapper::IntervalEnd() {
    ...
    auto retVal = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END, name);
    if (!retVal) {
        return;
    }
    auto bufferPair = retVal.value();
...
void SignpostWrapper::IntervalBeginAsync(const char* name, int32_t taskId) {
    ...
    auto retVal = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN_ASYNC, name, taskId);
    if (!retVal) {
        return;
    }
    auto bufferPair = retVal.value();
...
void SignpostWrapper::IntervalEndAsync(const char* name, int32_t taskId) {
    ...
    auto retVal = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC, name, taskId);
    if (!retVal) {
        return;
    }
    auto bufferPair = retVal.value();
...
void SignpostWrapper::EventEmit(const char* name, int64_t count) {
    ...
    auto retVal = FormatArgs(SignpostType::SIGNPOST_TYPE_EVENT, name, count);
    if (!retVal) {
        return;
    }
    auto bufferPair = retVal.value();
