...
void SignpostWrapper::IntervalEnd() {
    ...
    auto bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END, name);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
...
void SignpostWrapper::IntervalBeginAsync(const char* name, int32_t taskId) {
    ...
    auto bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN_ASYNC, name, taskId);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    if (bufferSize == 0 || buffer == nullptr) {
        return;
...
void SignpostWrapper::IntervalEndAsync(const char* name, int32_t taskId) {
    ...
    auto bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC, name, taskId);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    if (bufferSize == 0 || buffer == nullptr) {
        return;
...
void SignpostWrapper::EventEmit(const char* name, int64_t count) {
    ...
    auto bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_EVENT, name, count);
    size_t bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    if (bufferSize == 0 || buffer == nullptr) {
        return;
