...
void SignpostWrapper::IntervalBegin(const char* name) {
    ...
    emitWithNameImplFunc(&__dso_handle, osLog, spId, OS_SIGNPOST_INTERVAL_BEGIN, "Operation",
...
void SignpostWrapper::IntervalEnd() {
    ...
    void *buffer = bufferPair.second;
    __builtin_os_log_format(buffer, INTERVAL_FORMAT_STR, name);
    emitWithNameImplFunc(&__dso_handle, osLog, spId, OS_SIGNPOST_INTERVAL_END, "Operation",
                         INTERVAL_FORMAT_STR, static_cast<uint8_t*>(buffer), bufferSize);
    std::free(buffer);
}
...
void SignpostWrapper::IntervalBeginAsync(const char* name, int32_t taskId) {
    ...
    std::pair<unsigned long, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_BEGIN_ASYNC, name, taskId);
    unsigned long bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    __builtin_os_log_format(buffer, INTERVAL_ASYNC_FORMAT_STR, name, taskId);
    emitWithNameImplFunc(&__dso_handle, osLog, spId, OS_SIGNPOST_INTERVAL_BEGIN, "AsyncOperation",
...
void SignpostWrapper::IntervalEndAsync(const char* name, int32_t taskId) {
    ...
    std::pair<unsigned long, void *> bufferPair = FormatArgs(SignpostType::SIGNPOST_TYPE_INTERVAL_END_ASYNC, name, taskId);
    unsigned long bufferSize = bufferPair.first;
    void *buffer = bufferPair.second;
    __builtin_os_log_format(buffer, INTERVAL_ASYNC_FORMAT_STR, name, taskId);
    emitWithNameImplFunc(&__dso_handle, osLog, spId, OS_SIGNPOST_INTERVAL_END, "AsyncOperation",
                         INTERVAL_ASYNC_FORMAT_STR, static_cast<uint8_t*>(buffer), bufferSize);
    std::free(buffer);
}
...
void SignpostWrapper::EventEmit(const char* name, int64_t count) {
    ...
    emitWithNameImplFunc(&__dso_handle, osLog, spId, OS_SIGNPOST_EVENT , "OperationEmit",
