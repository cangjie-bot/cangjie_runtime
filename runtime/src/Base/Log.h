...
#define INTERVAL_FORMAT_STR "name:%{public}s"
#define INTERVAL_ASYNC_FORMAT_STR "name:%{public}s id: %d"
#define EVENT_FORMAT_STR "name:%{public}s, count: %lld"
#ifdef __cplusplus
extern "C" {
#endif
...
private:
    ...
    bool IsFuncAvailable(SignpostType type);
    bool IsIdValid(os_signpost_id_t spId);
    bool IsLogValid(os_log_t osLog);
    std::pair<unsigned long, void *> FormatArgs(SignpostType type, const char* name, int64_t value = 0);
