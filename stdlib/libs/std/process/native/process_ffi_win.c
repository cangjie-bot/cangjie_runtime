    ...
    if (!success) {
        ...
        if (err == ERROR_IO_PENDING) {
            ...
            while (true) {
                ...
                if (waitRes == WAIT_OBJECT_0) { // IO completed
                    if (!GetOverlappedResult(fd, &ovl, &numOfBytesRead, FALSE)) {
                        CloseHandle(ovl.hEvent);
                        err = GetLastError();
                        if (err == ERROR_BROKEN_PIPE) {
                    ...
                    break;
                } else if (waitRes == WAIT_OBJECT_0 + 1) { // process exited, wait IO to complete
                    ...
                    // keep waiting; do not cancel pending IO to avoid losing data
                    continue;
                } else { // WAIT_FAILED
