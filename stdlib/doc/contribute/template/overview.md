...
详见[xxx](./libs_template/std/xxx(package)/xxx_package_overview.md)。
...
- **必须严格按照上述格式**，每行 > 开头，留一个空格，说明字样用双星号包裹，留一个空行再写具体事项。否则OH网站上会显示错误。
...
- **情况1：直接扩展类型 A，扩展内容写在类型 A 中**
  ...
  详见[class 扩展示例1](./libs_template/std/xxx(package)/xxx_package_api/xxx_package_classes.md#extendt-class1t-where-t--comparable-直接对class1进行的扩展)。
...
- **情况2：为类型 A 扩展接口 B，A 在本包定义**
  ...
  详见[class 扩展示例2](./libs_template/std/xxx(package)/xxx_package_api/xxx_package_classes.md#extend-class1--b-where-t--comparable-为class1扩展接口b且class1在本包定义)。
  ...
- **情况3：为类型 A 扩展接口 B，B 在本包 A 不在**
  ...
  详见[interface 扩展示例](./libs_template/std/xxx(package)/xxx_package_api/xxx_package_interfaces.md#extend-bigintegert--interface1)。
