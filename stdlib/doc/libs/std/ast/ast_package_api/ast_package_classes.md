...
main(): Unit {
    ...
    // 创建 Argument 节点
    let arg1 = Argument()
    let argumentsList = ArrayList<Argument>([arg1])

    // 设置 arguments 属性
    annotation.arguments = argumentsList
    println("Number of arguments: ${annotation.arguments.size}")
}
```
...
main(): Unit {
    let annotation = Annotation()
    let atToken = Token(TokenKind.AT, "@")

    // 设置 at 属性
    annotation.at = atToken
...
```text
description: at, token_id: 51, token_literal_value: @, fileID: 1, line: 5, column: 19
```
...
main(): Unit {
    ...
    // 设置 attributes 属性
    annotation.attributes = quote(123)
    
    println("Attributes size: ${annotation.attributes.size}")
}
```
...
main(): Unit {
    ...
    println("annotaion.toTokens(): ${annotation.toTokens()}")
}
```
...
```text
description: colon, token_id: 28, token_literal_value: :, fileID: 1, line: 5, column: 22
```
...
main(): Unit {
    ...
    // 输出 expr 属性
    println("argument.expr.toTokens(): ${argument.expr.toTokens()}")
}
```
...
```text
description: identifier, token_id: 137, token_literal_value: arg, fileID: 1, line: 5, column: 27
```
...
main(): Unit {
    let argument = Argument()
    let keywordToken = Token(TokenKind.INOUT, "inout")

    // 设置 keyword 属性
    argument.keyword = keywordToken
    
    // 输出 keyword 属性
    argument.keyword.dump()
}
```
...
```text
description: inout, token_id: 129, token_literal_value: inout, fileID: 1, line: 5, column: 24
```
...
> **注意:**
>
> 编译时需要添加 `--experimental` 和 `--enbale-eh` 编译选项以支持 `Effect Handlers` 特性。
...
main(): Unit {
    ...
    // 添加 token
    tokens.append(Token(TokenKind.IDENTIFIER, "test"))
...
main(): Unit {
    ...
    println("Result tokens size: ${result.size}")
}
```
...
main(): Unit {
    ...
    println("tokens size before remove: ${tokens.size}")

    // 删除位置为 1 的 tokens
    tokens.remove(1)

    println("tokens size after remove: ${tokens.size}")
}
```
...
```text
tokens size before remove: 2
tokens size after remove: 1
```
...
```text
tokens[2..].dump():
description: class, token_id: 85, token_literal_value: class, fileID: 1, line: 5, column: 27
description: identifier, token_id: 137, token_literal_value: A, fileID: 1, line: 5, column: 33
description: l_curl, token_id: 6, token_literal_value: {, fileID: 1, line: 5, column: 35
description: r_curl, token_id: 7, token_literal_value: }, fileID: 1, line: 5, column: 36
```
