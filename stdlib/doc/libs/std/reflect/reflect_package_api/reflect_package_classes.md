...
功能：描述枚举构造子（case）信息，可用于查询构造子参数类型、注解，并根据构造子进行构造/拆解枚举实例。
...
- [EnumTypeInfo](#class-enumtypeinfo) - 构造子所属枚举类型的类型信息。
...
功能：获取该枚举构造子的名称（不包含包名前缀），格式通常为 `EnumName.CaseSignature`，例如 `E.M2<Int64>`。

返回值：

- [String](../../core/core_package_api/core_package_structs.md#struct-string) - 构造子的名称。
...
功能：获取该枚举构造子的限定名称（包含包名前缀），格式通常为 `package.EnumName.CaseSignature`，例如 `default.E.M2<Int64>`。

返回值：

- [String](../../core/core_package_api/core_package_structs.md#struct-string) - 构造子的限定名称。
...
- [InfoNotFoundException](reflect_package_exceptions.md#class-infonotfoundexception) - 如果无法找到对应的枚举构造子，则抛出异常。
- [IllegalTypeException](reflect_package_exceptions.md#class-illegaltypeexception) - 如果 `qualifiedName` 对应的类型不是枚举类型或枚举构造子类型，则抛出异常。
...
- [InvocationTargetException](reflect_package_exceptions.md#class-invocationtargetexception) - 如果 `instance` 不是该枚举的实例，则抛出异常。
...
main(): Unit {
    let ctor = EnumConstructorInfo.of(E.M2(0))
    let inst = (ctor.call([42]) as E).getOrThrow()
    match (inst) {
        case E.M2(v) => println(v)
...
> **注意：**
>
> `instance` 的构造子必须与该 [EnumConstructorInfo](#class-enumconstructorinfo) 一致，否则会抛出异常。
...
- [IllegalTypeException](reflect_package_exceptions.md#class-illegaltypeexception) - 如果 `instance` 的构造子与该构造子不一致，则抛出异常。
...
> **说明：**
>
> `qualifiedName` 支持传入枚举类型的限定名称或枚举构造子类型的限定名称，例如：
>
> - `default.E`
> - `default.E.M1`
> - `default.E.M2<Int64>`
...
- [InfoNotFoundException](reflect_package_exceptions.md#class-infonotfoundexception) - 如果无法获取与 `qualifiedName` 匹配的类型信息，则抛出异常。
- [IllegalTypeException](reflect_package_exceptions.md#class-illegaltypeexception) - 如果获取到的类型信息不是枚举类型，则抛出异常。
...
main(): Unit {
    ...
    let e2 = EnumTypeInfo.get("test.E.M2<Int64>")
    println(e1.qualifiedName)
    println(e2.qualifiedName)
    return
}
```
...
- [IllegalTypeException](reflect_package_exceptions.md#class-illegaltypeexception) - 如果 `instance` 不属于任何枚举类型，则抛出异常。
...
- [IllegalTypeException](reflect_package_exceptions.md#class-illegaltypeexception) - 如果 `T` 不属于任何枚举类型，则抛出异常。
...
> **说明：**
>
> - 当构造子没有参数时，`constructor` 为构造子名，例如 `M1`。
> - 当构造子有参数时，`constructor` 为签名，例如 `M2<Int64>`、`M3<Int64, String>`，其中参数类型使用限定名称。
...
- [InfoNotFoundException](reflect_package_exceptions.md#class-infonotfoundexception) - 如果找不到对应的构造子签名，则抛出异常。
- [InvocationTargetException](reflect_package_exceptions.md#class-invocationtargetexception) - 如果 `args` 的数量或类型与构造子参数不匹配，则抛出异常。
...
main(): Unit {
    ...
    let ctor = e.getConstructor("M3", argsCount: 2)
    let inst = (e.construct(ctor.signature, [42, "abc"]) as E).getOrThrow()
    match (inst) {
...
- [InvocationTargetException](reflect_package_exceptions.md#class-invocationtargetexception) - 如果 `instance` 不是该枚举的实例，则抛出异常。
- [InfoNotFoundException](reflect_package_exceptions.md#class-infonotfoundexception) - 如果无法匹配到对应的构造子签名，则抛出异常。
...
- [TypeInfo](#class-typeinfo) - 返回值类型信息。
