...
main(): Int64 {
    ...
    // 发送SIGUSR1信号给自己（用户自定义信号，通常不会终止进程）
    let result = kill(pid, SIGUSR1)
    if (result == 0) {
        println("Successfully sent SIGUSR1 signal to process.")
    } else {
        println("Failed to send SIGUSR1 signal to process.")
    }
...
```text
Successfully sent SIGUSR1 signal to process.
```
